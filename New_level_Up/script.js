...
// COMPLETE VERSION WITH ENHANCED RANK SYSTEM & PLAYER CARD FEATURE
// =========================
...
const gameData = {
    ...
    // NEW: Rank system configuration
    rankSystem: {
        ranks: [
            { name: "E-RANK HUNTER", level: 1, color: "#8888aa", rewards: { statPoints: 0, gold: 0 } },
            { name: "D-RANK HUNTER", level: 10, color: "#9c88ff", rewards: { statPoints: 2, gold: 500, stats: { strength: 2, intelligence: 2, agility: 2 } } },
            { name: "C-RANK HUNTER", level: 20, color: "#00a8ff", rewards: { statPoints: 3, gold: 1000, stats: { strength: 3, intelligence: 3, agility: 3 } } },
            { name: "B-RANK HUNTER", level: 30, color: "#44bd32", rewards: { statPoints: 5, gold: 2000, stats: { strength: 5, intelligence: 5, agility: 5 } } },
            { name: "A-RANK HUNTER", level: 40, color: "#fbc531", rewards: { statPoints: 8, gold: 4000, stats: { strength: 8, intelligence: 8, agility: 8 } } },
            { name: "S-RANK HUNTER", level: 50, color: "#e84118", rewards: { statPoints: 15, gold: 8000, stats: { strength: 15, intelligence: 15, agility: 15 } } }
        ]
    }
};
...
// NEW: Rank system elements
const rankUpOverlay = document.getElementById('rankUpOverlay');
const newRankEl = document.getElementById('newRank');
...
const rankProgressTextEl = document.getElementById('rankProgressText');
const nextRankEl = document.getElementById('nextRank');
const rankProgressEl = document.getElementById('rankProgress');
const rankMarkerEl = document.getElementById('rankMarker');

// NEW: Player Card elements - ADDED
...
function updatePlayerRank() {
    ...
    const rankSystem = gameData.rankSystem;
    
    // Find current rank and next rank
    let currentRank = rankSystem.ranks[0];
    let nextRank = null;
    
    for (let i = rankSystem.ranks.length - 1; i >= 0; i--) {
        if (player.level >= rankSystem.ranks[i].level) {
            currentRank = rankSystem.ranks[i];
            if (i < rankSystem.ranks.length - 1) {
                nextRank = rankSystem.ranks[i + 1];
            }
            break;
        }
    }
    ...
    // Update player rank
    player.rank = currentRank.name;
    
    // Calculate rank progress
    let progress = 0;
    if (nextRank) {
        const levelRange = nextRank.level - currentRank.level;
        const currentProgress = player.level - currentRank.level;
        progress = Math.min(100, Math.round((currentProgress / levelRange) * 100));
        
        // Update next rank display
        if (nextRankEl) {
            nextRankEl.textContent = `Next: ${nextRank.name}`;
        }
    } else {
        // Max rank reached
        progress = 100;
        if (nextRankEl) {
            nextRankEl.textContent = "MAX RANK";
        }
    }
    ...
    if (playerRankEl) {
        ...
        playerRankEl.className = `player-rank rank-${player.rank.charAt(0).toLowerCase()}`;
    }
    
    // Update rank progress bar
    if (rankProgressEl) {
        rankProgressEl.style.width = `${progress}%`;
    }
    
    // Update rank marker
    if (rankMarkerEl) {
        rankMarkerEl.style.left = `${progress}%`;
    }
    
    // Update rank labels
    if (currentRankLabelEl) {
        currentRankLabelEl.textContent = currentRank.name.charAt(0);
        currentRankLabelEl.style.color = currentRank.color;
    }
    
    if (nextRankLabelEl) {
        if (nextRank) {
            nextRankLabelEl.textContent = nextRank.name.charAt(0);
            nextRankLabelEl.style.color = nextRank.color;
        } else {
            nextRankLabelEl.textContent = "MAX";
            nextRankLabelEl.style.color = "#e84118";
        }
    }
    
    // Update progress text
    if (rankProgressTextEl) {
        if (nextRank) {
            rankProgressTextEl.textContent = `${progress}% to next rank (Level ${player.level}/${nextRank.level})`;
        } else {
...
function updateUnlocks() {
    ...
    unlocksList.forEach((item, index) => {
        ...
        if (player.level >= requiredLevel) {
            ...
            const featureName = item.textContent.replace(` (Level ${requiredLevel})`, '').trim();
...
function showRankUp(oldRank, newRank, rewards) {
    const rankObj = gameData.rankSystem.ranks.find(r => r.name === newRank);
    
    if (!rankObj) return;
    
    // Update overlay content
    newRankEl.textContent = newRank;
    rankDisplayEl.textContent = newRank;
    newRankEl.style.color = rankObj.color;
    
    // Clear previous rewards
    rewardsListEl.innerHTML = '';
    
    // Add rewards to list
    if (rewards.statPoints && rewards.statPoints > 0) {
        ...
        rewardEl.innerHTML = `
            ...
            <div class="reward-amount">+${rewards.statPoints}</div>
        `;
    ...
    if (rewards.stats) {
        if (rewards.stats.strength && rewards.stats.strength > 0) {
            const rewardEl = document.createElement('div');
            rewardEl.className = 'reward-item';
            rewardEl.innerHTML = `
                <div class="reward-name">
                ...
                <div class="reward-amount">+${rewards.stats.strength}</div>
            `;
            rewardsListEl.appendChild(rewardEl);
        }
        
        if (rewards.stats.intelligence && rewards.stats.intelligence > 0) {
            const rewardEl = document.createElement('div');
            rewardEl.className = 'reward-item';
            rewardEl.innerHTML = `
                <div class="reward-name">
                ...
                <div class="reward-amount">+${rewards.stats.intelligence}</div>
            `;
            rewardsListEl.appendChild(rewardEl);
        }
        
        if (rewards.stats.agility && rewards.stats.agility > 0) {
            const rewardEl = document.createElement('div');
            rewardEl.className = 'reward-item';
            rewardEl.innerHTML = `
                <div class="reward-name">
                ...
                <div class="reward-amount">+${rewards.stats.agility}</div>
            `;
            rewardsListEl.appendChild(rewardEl);
        }
    }
...
function applyRankBonuses(oldRank, newRank) {
    ...
    // Find the rank object
    const rankObj = rankSystem.ranks.find(r => r.name === newRank);
    
    if (rankObj && rankObj.rewards) {
        const rewards = rankObj.rewards;
        
        // Apply stat points
        if (rewards.statPoints && rewards.statPoints > 0) {
            player.statPoints += rewards.statPoints;
        }
        ...
        // Apply stat bonuses
        if (rewards.stats) {
            if (rewards.stats.strength) player.stats.strength += rewards.stats.strength;
            if (rewards.stats.intelligence) player.stats.intelligence += rewards.stats.intelligence;
            if (rewards.stats.agility) player.stats.agility += rewards.stats.agility;
        }
        
        // Show rank up overlay
        showRankUp(oldRank, newRank, rewards);
        
        addSystemMessage("[RANK SYSTEM]", `Rank Up! ${newRank} reached — ${rewards.statPoints || 0} stat points, ${rewards.gold || 0} gold awarded!`);
        addActivityLog(`Reached ${newRank} — Rank up rewards claimed`, "fa-award");
...
// =========================
// PLAYER CARD SYSTEM - NEW ADDED
// =========================

// Generate player card HTML
function generatePlayerCard() {
    const player = gameData.player;
    const currentRank = updatePlayerRank().currentRank;
    const rankLetter = player.rank.charAt(0).toLowerCase();
    const requiredXP = calculateRequiredXP(player.level);
    const xpPercent = (player.xp / requiredXP) * 100;
    ...
    const cardHTML = `
        <div class="player-card" id="playerCard">
            <div class="player-card-header">
                <div class="player-card-title">SOLO LEVELING SYSTEM</div>
                <div class="player-card-name">${player.name}</div>
                <div class="player-card-rank rank-${rankLetter}">${player.rank}</div>
            ...
            <div class="player-card-body">
                <div class="player-card-section">
                    ...
                    <div class="stats-grid">
                        <div class="stat-item">
                            ...
                            <div class="stat-value">${player.stats.strength}</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-label">INT</div>
                            <div class="stat-value">${player.stats.intelligence}</div>
                        </div>
                        <div class="stat-item">
                        ...
                        <div class="stat-item">
                            ...
                            <div class="stat-value">${player.totalStats}</div>
                        </div>
                    </div>
                </div>
                <div class="player-card-section">
                    ...
                    <div class="stats-grid">
                        <div class="stat-item">
                            ...
                            <div class="stat-value">${player.totalQuestsCompleted}</div>
                        </div>
                        <div class="stat-item">
                        ...
                        <div class="stat-item">
                            ...
                            <div class="stat-value">${player.daysActive}</div>
                        </div>
                        <div class="stat-item">
                ...
                <div class="player-card-section progress-section">
                    <h3><i class="fas fa-star"></i> EXPERIENCE</h3>
                    <div class="xp-display-card">
                        <div style="display:flex; justify-content:space-between; margin-bottom:5px; font-size:12px;">
                            <span>${player.xp} / ${requiredXP} XP</span>
                            <span>${xpPercent.toFixed(1)}%</span>
                        </div>
                        <div class="xp-bar-card">
                            <div class="xp-fill-card" style="width: ${xpPercent}%"></div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="player-card-footer">
                <div class="player-card-date">${new Date().toLocaleDateString('en-US', { 
                    year: 'numeric', 
                    month: 'short', 
                    day: 'numeric' 
                })}</div>
                <div class="player-card-id">ID: ${Date.now().toString(36).toUpperCase()}</div>
            </div>
        </div>
    `;
...
function showPlayerCard() {
    // Check if player is D-rank or higher
    const playerRank = gameData.player.rank;
    const rankLetter = playerRank.charAt(0).toUpperCase();
    
    // Only allow D-rank and higher to download
    const canDownload = rankLetter > 'E';
    
    // Generate card HTML
    playerCardContainer.innerHTML = generatePlayerCard();
    
    // Update download button state
    downloadCardBtn.disabled = !canDownload;
    downloadCardBtn.innerHTML = canDownload ? 
        '<i class="fas fa-download"></i> DOWNLOAD CARD' : 
        '<i class="fas fa-lock"></i> REACH D-RANK TO DOWNLOAD';
    
    // Show modal
    cardModal.classList.add('active');
    
    // Add system message
    addSystemMessage("[PLAYER CARD]", "Player card displayed" + (canDownload ? ". You can download your card!" : ". Reach D-rank to download."));
}

// Download player card as image
function downloadPlayerCard() {
    const playerRank = gameData.player.rank;
    const rankLetter = playerRank.charAt(0).toUpperCase();
    
    // Double-check rank requirement
    if (rankLetter <= 'E') {
        addSystemMessage("[PLAYER CARD]", "You must reach D-rank or higher to download your card!");
        return;
    }
    ...
    if (!cardElement) {
        addSystemMessage("[PLAYER CARD]", "Error: Card not found!");
        return;
    }
    ...
    if (typeof html2canvas === 'undefined') {
        ...
        script.onload = function() {
            captureAndDownloadCard(cardElement);
        };
        script.onerror = function() {
            addSystemMessage("[PLAYER CARD]", "Failed to load image converter. Please try again later.");
        };
...
// Capture and download the card
function captureAndDownloadCard(cardElement) {
    html2canvas(cardElement, {
        ...
        link.download = `SoloLeveling_${gameData.player.name.replace(/\s+/g, '_')}_Card.png`;
        link.href = image;
        ...
        // Add system message
        addSystemMessage("[PLAYER CARD]", `Player card downloaded: ${link.download}`);
        addActivityLog(`Downloaded player card`, "fa-id-card");
    }).catch(error => {
        console.error('Error capturing card:', error);
        addSystemMessage("[PLAYER CARD]", "Failed to download card. Please try again.");
    });
}
...
function checkLevelUp(oldLevel) {
    ...
    while (player.xp >= calculateRequiredXP(player.level)) {
        ...
        if (oldRank !== player.rank) {
            applyRankBonuses(oldRank, player.rank);
        }
    ...
    // Always update rank progress (even if no level up)
    updatePlayerRank();
...
function loadGameData() {
    try {
        ...
        if (savedData) {
            ...
            // Ensure rank system exists
            if (!gameData.rankSystem) {
                gameData.rankSystem = {
                    ranks: [
                        { name: "E-RANK HUNTER", level: 1, color: "#8888aa", rewards: { statPoints: 0, gold: 0 } },
                        { name: "D-RANK HUNTER", level: 10, color: "#9c88ff", rewards: { statPoints: 2, gold: 500, stats: { strength: 2, intelligence: 2, agility: 2 } } },
...
function initEventListeners() {
    ...
    // Player Card buttons - NEW ADDED
    if (showCardBtn) {
        showCardBtn.addEventListener('click', showPlayerCard);
    }
    
    if (downloadCardBtn) {
        downloadCardBtn.addEventListener('click', downloadPlayerCard);
    }
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            ...
            cardModal.classList.remove('active'); // NEW ADDED
        }
    });
